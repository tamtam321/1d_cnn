...
def startNN():
    ...
    while block < 5:
        ...
        # model.add(layers.Conv2D(filters=40, kernel_size=(1, 30), strides=(1, 1), padding="same", activation="relu", input_shape=(64, 321, 1)))  # temporal convolution (64, 321, 1) - (NEEG, N, 1)
        # model.add(layers.Conv2D(filters=40, kernel_size=(64, 1), strides=(1, 1), padding="valid", activation="relu"))  # spatial conv # input_shape=(64, 321, 40) - (NEEG, N, 40)
        ...
        model.add(layers.Conv1D(filters=32, kernel_size=20, padding="valid", activation="relu"))
        model.add(layers.BatchNormalization())
        model.add(layers.SpatialDropout1D(rate=0.2))
        model.add(layers.Conv1D(filters=32, kernel_size=6, padding="valid", activation="relu"))
        model.add(layers.AvgPool1D(pool_size=2))
        model.add(layers.Conv1D(filters=32, kernel_size=6, padding="valid", activation="relu"))
        model.add(layers.SpatialDropout1D(rate=0.2))
        model.add(layers.Flatten())
        model.add(layers.Dense(units=296, activation="relu"))
        model.add(layers.Dropout(rate=0.25))
        model.add(layers.Dense(units=148, activation="relu"))
        model.add(layers.Dropout(rate=0.25))
        ...
        # tanítás
        model.fit(learning_data_set[0], learning_data_set[1], epochs=epochs,
